...
class v1_mi6_env2_mod_new_stack_solo(nn.Module):
    def __init__(self, output_nbr, args):
        super(v1_mi6_env2_mod_new_stack_solo, self).__init__()
    ...
    def _create_cnn_block(self):
        return nn.Sequential(
            ...
            custom2DCNN(80, 80, (2, 7), "same"),
            nn.MaxPool2d(2), # 17, 28
            nn.Dropout2d(0.25),
            custom2DCNN(80, 160, (2, 5), "same"),
            nn.MaxPool2d(2), # 8, 14
            nn.Dropout2d(0.25),
            custom2DCNN(160, 160, (2, 3), "same"),
            nn.MaxPool2d(2), # 4, 7
    ...
    def forward(self, x):
        ...
        # c1 = torch.cat((x4_1), dim=1)
        # c2 = torch.cat((x4_2), dim=1)
        # c3 = torch.cat((x4_3), dim=1)
        # c4 = torch.cat((x4_4), dim=1)
        # c5 = torch.cat((x4_5), dim=1)
        # c6 = torch.cat((x4_6), dim=1)
...
class v1_mi6_env2_mod_new_stack_mix(nn.Module):
    def __init__(self, output_nbr, args):
        super(v1_mi6_env2_mod_new_stack_mix, self).__init__()
    ...
    def forward(self, x):
        ...
        x0_1, x0_2, x0_3, x0_4, x0_5, x0_6 = torch.split(self.logmel2(x)[:,:,:, :15], 70, dim=2)
        x1_1, x1_2, x1_3, x1_4, x1_5, x1_6 = torch.split(self.logmel2(x)[:,:,:, 14:29], 70, dim=2)
        x2_1, x2_2, x2_3, x2_4, x2_5, x2_6 = torch.split(self.logmel2(x)[:,:,:, 28:43], 70, dim=2)
        x3_1, x3_2, x3_3, x3_4, x3_5, x3_6 = torch.split(self.logmel2(x)[:,:,:, 42:], 70, dim=2)
        ...
        c1 = torch.cat((x0_1, x1_1, x2_1, x3_1, x4_1, x5_1, x6_1, x7_1), dim=1)
        c2 = torch.cat((x0_2, x1_2, x2_2, x3_2, x4_2, x5_2, x6_2, x7_2), dim=1)
        c3 = torch.cat((x0_3, x1_3, x2_3, x3_3, x4_3, x5_3, x6_3, x7_3), dim=1)
        c4 = torch.cat((x0_4, x1_4, x2_4, x3_4, x4_4, x5_4, x6_4, x7_4), dim=1)
        c5 = torch.cat((x0_5, x1_5, x2_5, x3_5, x4_5, x5_5, x6_5, x7_5), dim=1)
        c6 = torch.cat((x0_6, x1_6, x2_6, x3_6, x4_6, x5_6, x6_6, x7_6), dim=1)
...
class ARNModel_mix(nn.Module):
    def __init__(self, output_nbr, args):
        ...
        self.sr = args.sr
        self.logmel = LogMelSpectrogramLayer(n_fft=4096, hop_length=128, sample_rate=self.sr, n_mels=420)
        self.output_nbr = output_nbr

        self.env1 = EnvelopeFollowingLayerTorchScript(n_fft=4096, hop_length=128, smoothing_factor=4)
    ...
    def _create_env_block(self):
        return nn.Sequential(
            customARB1D(1, 40, 20, 1, 8, first=True),
            customARB1D(40, 40, 15, 2, 8),
            customARB1D(40, 40, 10, 2, 8),
            customARB1D(40, 80, 7, 3, 7),
            customARB1D(80, 160, 2, 4, 2)
        )
